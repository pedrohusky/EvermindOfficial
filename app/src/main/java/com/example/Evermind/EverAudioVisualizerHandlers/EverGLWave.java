 ...
 */
class EverGLWave extends EverGLShape {
    ...
    /**
     * Smooth coefficient for {@link EverAudioUtils#smooth(float, float, float)} method.
     */
    ...
    public EverGLWave(float[] color, float fromX, float toX, float fromY, float toY, byte direction, Random random) {
        super(color);
        this.fromX = fromX;
        this.toX = toX;
        this.fromY = fromY;
    ...
    private void initVertices() {
        ...
        // center
        vertices[0] = EverAudioUtils.normalizeGl(0f, fromX, toX);
        vertices[1] = EverAudioUtils.normalizeGl(-1f, fromY, toY);

        // left bottom footer
        vertices[3] = EverAudioUtils.normalizeGl(-1f, fromX, toX);
        vertices[4] = EverAudioUtils.normalizeGl(-1f, fromY, toY);

        // left top footer
        vertices[6] = vertices[3];
        vertices[7] = EverAudioUtils.normalizeGl(0f, fromY, toY);

        // right top footer
        vertices[vertices.length - 6] = EverAudioUtils.normalizeGl(1f, fromX, toX);
    ...
    public void update(float dAngle) {
        ...
        if (coefficient == 0 && latestCoefficient > 0) {
            coefficient = EverAudioUtils.smooth(0, latestCoefficient, SMOOTH_A);
        }

        float val = (float) Math.sin(angle) * coefficient;
        if (prevVal > 0 && val <= 0 || prevVal < 0 && val >= 0) {
            coefficient = EverAudioUtils.smooth(coefficient, latestCoefficient, SMOOTH_A);
            waveX = random.nextFloat() * 0.3f * (random.nextBoolean() ? 1 : -1);
        }
        prevVal = val;
        int i = 0;
        double step = 1.0 / POINTS_PER_WAVE;
        float posX = EverAudioUtils.normalizeGl(waveX, fromX, toX);
        float posY = EverAudioUtils.normalizeGl(val, fromY, toY);
        for (float time = 0; time < 1 - step / 2; time += step) {
            vertices[COORDS_PER_VERTEX * i + 1 + SKIP] = angle;
            vertexBuffer.put(COORDS_PER_VERTEX * i + SKIP, EverAudioUtils.quad(time, vertices[6], posX, vertices[vertices.length - 6]));
            vertexBuffer.put(COORDS_PER_VERTEX * i + 1 + SKIP, EverAudioUtils.quad(time, vertices[7], posY, vertices[vertices.length - 5]));
            i++;
        }
    }
