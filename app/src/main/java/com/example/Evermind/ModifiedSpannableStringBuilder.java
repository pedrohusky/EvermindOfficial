...
public class ModifiedSpannableStringBuilder implements CharSequence, GetChars, Editable, Spannable, Appendable {

    private final static String TAG = "MSpannableStringBuilder";

    /**
     * Create a new ModifiedSpannableStringBuilder with empty contents
     */
    ...
    /**
     * Create a new ModifiedSpannableStringBuilder containing a copy of the
     * specified text, including its spans if any.
     */
    public ModifiedSpannableStringBuilder(CharSequence text) {
     ...
     */
    public ModifiedSpannableStringBuilder(CharSequence text, int start, int end) {
        int srclen = end - start;

        if (srclen < 0) throw new StringIndexOutOfBoundsException();

        mText = ModifiedArrayUtils.newUnpaddedCharArray(ModifiedGrowingArrayUtils.growSize(srclen));
        mGapStart = srclen;
        ...
        mSpanCount = 0;
        mSpans = ModifiedEmptyArray.OBJECT;
        mSpanStarts = ModifiedEmptyArray.INT;
        mSpanEnds = ModifiedEmptyArray.INT;
        mSpanFlags = ModifiedEmptyArray.INT;
    ...
    public static ModifiedSpannableStringBuilder valueOf(CharSequence source) {
        if (source instanceof ModifiedSpannableStringBuilder) {
            return (ModifiedSpannableStringBuilder) source;
        } else {
            return new ModifiedSpannableStringBuilder(source);
        }
    }
    ...
    private void resizeFor(int size) {
        ...
        char[] newText = ModifiedArrayUtils.newUnpaddedCharArray(ModifiedGrowingArrayUtils.growSize(size));
    ...
    public ModifiedSpannableStringBuilder insert(int where, CharSequence tb, int start, int end) {
        return replace(where, where, tb, start, end);
    }

    // Documentation from interface
    public ModifiedSpannableStringBuilder insert(int where, CharSequence tb) {
        return replace(where, where, tb, 0, tb.length());
    }

    // Documentation from interface
    public ModifiedSpannableStringBuilder delete(int start, int end) {
        ModifiedSpannableStringBuilder ret = replace(start, end, "", 0, 0);
    ...
    // Documentation from interface
    public ModifiedSpannableStringBuilder append(CharSequence text) {
        int length = length();
     ...
     * @return this {@code ModifiedSpannableStringBuilder}.
     */
    public ModifiedSpannableStringBuilder append(CharSequence text, Object what, int flags) {
        int start = length();
        append(text);
    ...
    // Documentation from interface
    public ModifiedSpannableStringBuilder append(CharSequence text, int start, int end) {
        int length = length();
    ...
    // Documentation from interface
    public ModifiedSpannableStringBuilder replace(int start, int end, CharSequence tb) {
        return replace(start, end, tb, 0, tb.length());
    }

    // Documentation from interface
    public ModifiedSpannableStringBuilder replace(final int start, final int end,
    ...
    private void setSpan(boolean send, Object what, int start, int end, int flags) {
        ...
        mSpans = ModifiedGrowingArrayUtils.append(mSpans, mSpanCount, what);
        mSpanStarts = ModifiedGrowingArrayUtils.append(mSpanStarts, mSpanCount, start);
        mSpanEnds = ModifiedGrowingArrayUtils.append(mSpanEnds, mSpanCount, end);
        mSpanFlags = ModifiedGrowingArrayUtils.append(mSpanFlags, mSpanCount, flags);
        mSpanCount++;
    ...
    public <T> T[] getSpans(int queryStart, int queryEnd, Class<T> kind) {
        if (kind == null) return ModifiedArrayUtils.emptyArray(kind);
        ...
        if (count == 0) {
            return ModifiedArrayUtils.emptyArray(kind);
        }
        if (count == 1) {
            // Safe conversion, but requires a suppressWarning
