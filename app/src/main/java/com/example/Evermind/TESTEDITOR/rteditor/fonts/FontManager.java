    ...
    private static Map<String, String> getAssetFonts(@NonNull Context context) {
        synchronized (ASSET_FONTS_BY_NAME) {
            ...
            if (ASSET_FONTS_BY_NAME.isEmpty()) {
                ...
                Collection<String> fontFiles = com.example.Evermind.TESTEDITOR.rteditor.fonts.AssetIndex.getAssetIndex(context);
                ...
                for (String filePath : fontFiles) {
                    if (filePath.toLowerCase(Locale.getDefault()).endsWith("ttf")) {
                        String fontName = com.example.Evermind.TESTEDITOR.rteditor.fonts.TTFAnalyzer.getFontName(assets, filePath);
                        if (fontName == null) {
            ...
            for (String fontDir : FONT_DIRS) {
                ...
                for (File file : files) {
                    ...
                    if (!SYSTEM_FONTS_BY_PATH.containsKey(filePath)) {
                        String fontName = com.example.Evermind.TESTEDITOR.rteditor.fonts.TTFAnalyzer.getFontName(file.getAbsolutePath());
                        if (fontName == null) {
                            fontName = getFileName(filePath);
                        }
