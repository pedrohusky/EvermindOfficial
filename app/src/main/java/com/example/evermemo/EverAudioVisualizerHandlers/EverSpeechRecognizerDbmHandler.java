 ...
 */
public class EverSpeechRecognizerDbmHandler extends DbmHandler<Float> implements RecognitionListener {
     ...
     */
    EverSpeechRecognizerDbmHandler(@NonNull Context context) {
     ...
     */
    EverSpeechRecognizerDbmHandler(@NonNull Context context, float minRmsDbValue, float maxRmsDbValue) {
    ...
    protected void onDataReceivedImpl(Float rmsdB, int layersCount, float[] dBmArray, float[] ampsArray) {
        for (int i = 0; i < layersCount; i++) {
            dBmArray[i] = EverAudioUtils.normalize(rmsdB, minRmsDbValue, maxRmsDbValue);
            ampsArray[i] = 1;
        }
    }
