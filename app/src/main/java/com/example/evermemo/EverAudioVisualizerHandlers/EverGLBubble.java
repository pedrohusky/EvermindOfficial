 ...
 */
class EverGLBubble extends EverGLShape {
    ...
    public void update(long dt, float ratioY) {
        ...
        vertexBuffer.put(0, EverAudioUtils.normalizeGl(0, fromX, toX));
        vertexBuffer.put(1, EverAudioUtils.normalizeGl(centerY * ratioY, fromY, toY));
        for (int i = 1; i <= POINTS_PER_CIRCLE; i++) {
            vertexBuffer.put(COORDS_PER_VERTEX * i, EverAudioUtils.normalizeGl((float) Math.sin(-Math.PI + step * i), fromX, toX));
            vertexBuffer.put(COORDS_PER_VERTEX * i + 1, EverAudioUtils.normalizeGl((float) Math.cos(-Math.PI + step * i) * ratioY, fromY, toY));
        }
        this.fromY = fromY;
    }
