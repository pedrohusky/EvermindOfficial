...
class EverWave : View {
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        val ta = context.obtainStyledAttributes(attrs, R.styleable.EverWave)

        waveWidth = ta.getDimension(R.styleable.EverWave_wave_width, waveWidth)
        waveGap = ta.getDimension(R.styleable.EverWave_wave_gap, waveGap)
        waveCornerRadius =
            ta.getDimension(R.styleable.EverWave_wave_corner_radius, waveCornerRadius)
        waveMinHeight = ta.getDimension(R.styleable.EverWave_wave_min_height, waveMinHeight)
        waveBackgroundColor =
            ta.getColor(R.styleable.EverWave_wave_background_color, waveBackgroundColor)
        waveProgressColor = ta.getColor(R.styleable.EverWave_wave_progress_color, waveProgressColor)
        progress = ta.getInteger(R.styleable.EverWave_wave_progress, progress)
        val gravity = ta.getString(R.styleable.EverWave_wave_gravity)
    ...
    interface SeekBarOnProgressChanged {
        ...
        fun onProgressChanged(EverWave: EverWave, progress: Int, fromUser: Boolean)
    }
    ...
    var waveCornerRadius: Float = dp(context, 2)
        set(value) {
            field = value
            invalidate()
        }
    ...
    fun setMaxValue(progress: Int) {
        this.mMaxValue = progress
    }
